...
async function getFontBufferSource(e, s, l) {
  ...
  const { buffer: G, zone: i } = await getFontPitch(e, r, l), n = l.createBufferSource();
  n.buffer = G;
  const u = i.originalPitch - 100 * i.coarseTune - i.fineTune, o = 1 * Math.pow(2, (100 * r - u) / 1200);
  return n.playbackRate.value = o, i.loopStart > 1 && i.loopStart < i.loopEnd && (n.loop = !0, n.loopStart = i.loopStart / i.sampleRate, n.loopEnd = i.loopEnd / i.sampleRate), n;
}
let bufferCache = {};
...
async function getBuffer(e, s) {
  if (e.sample) {
    ...
    for (var l = 0; l < f.length / 2; l++)
      r = f.charCodeAt(l * 2), G = f.charCodeAt(l * 2 + 1), r < 0 && (r = 256 + r), G < 0 && (G = 256 + G), i = G * 256 + r, i >= 65536 / 2 && (i = i - 65536), a[l] = i / 65536;
  } else if (e.file) {
    const f = e.file.length, a = new ArrayBuffer(f), r = new Uint8Array(a), G = atob(e.file);
    let i;
    for (let n = 0; n < G.length; n++)
      i = G.charCodeAt(n), r[n] = i;
...
function registerSoundfonts() {
  Object.entries(gm).forEach(([e, s]) => {
    registerSound(
      e,
      async (l, f, a) => {
        const [r, G, i, n] = getADSRValues([
          f.attack,
          f.decay,
          f.sustain,
          f.release
        ]), { duration: u } = f, o = getSoundIndex(f.n, s.length), S = s[o], p = getAudioContext(), t = await getFontBufferSource(S, f, p);
        t.start(l);
        const U = p.createGain(), C = t.connect(U), d = l + u;
        getParamADSR(C.gain, r, G, i, n, 0, 0.3, l, d, "linear");
        let m = d + n + 0.01, F = getVibratoOscillator(t.detune, f, l);
        getPitchEnvelope(t.detune, f, l, d), t.stop(m);
        const R = (M) => {
        };
        return t.onended = () => {
          t.disconnect(), F?.stop(), C.disconnect(), a();
        }, { node: C, stop: R };
      },
...
Pattern.prototype.soundfont = function(e, s = 0) {
  return this.onTrigger((l, f, a) => {
    const r = getAudioContext(), G = getPlayableNoteValue(f), i = e.presets[s % e.presets.length], n = r.currentTime + l - a, u = [r, i, noteToMidi(G), n];
    startPresetNote(...u)(n + f.duration);
  });
};
